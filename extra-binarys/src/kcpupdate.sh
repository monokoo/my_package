...
if [ "$latest_version" != "$local_version" ]; then
	...
	wget --no-check-certificate --timeout=8 -t 2 https://github.com/xtaci/kcptun/releases/download/v$latest_version/kcptun-linux-arm-$latest_version.tar.gz -O /tmp/kcptun-linux-arm/kcptun-linux-arm-$latest_version.tar.gz
	[ ! -s "/tmp/kcptun-linux-arm/kcptun-linux-arm-$latest_version.tar.gz" ] && echo "下载失败，请稍候重试！" && exit 0
