...
mwan3_rtmon_ipv6()
{
	...
	local ret=1
	mkdir -p /tmp/mwan3rtmon
	$IP6 route list table main  | grep -v "^default\|^::/0" | sort -n >/tmp/mwan3rtmon/ipv6.main
	while uci get mwan3.@interface[$idx] >/dev/null 2>&1 ; do
		idx=$((idx+1))
		tid=$idx
		[ "$(uci get mwan3.@interface[$((idx-1))].family)" = "ipv6" ] && {
			if $IP6 route list table $tid | grep -q "^default\|^::/0"; then
				$IP6 route list table $tid  | grep -v "^default\|^::/0" | sort -n >/tmp/mwan3rtmon/ipv6.$tid
				diff -u /tmp/mwan3rtmon/ipv6.$tid /tmp/mwan3rtmon/ipv6.main | grep ^-[^-] | sed 's/^-//' | while read line; do
	...
	if [ -n "$use_policy" ]; then
		...
		if [ "$family" == "any" ]; then
			...
			for IPT in "$IPT4" "$IPT6"; do
				case $proto in
					tcp|udp)
					[ "$enable" == "1" ] && $IPT -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m multiport --sports $src_port -m multiport --dports $dest_port -m mark --mark 0/$MMX_MASK -m comment --comment "$1" -j $policy &> /dev/null
					;;
					*)
					[ "$enable" == "1" ] && $IPT -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m mark --mark 0/$MMX_MASK -m comment --comment "$1" -j $policy &> /dev/null
					;;
				esac
			done
			...
			case $proto in
				tcp|udp)
				[ "$enable" == "1" ] && $IPT4 -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m multiport --sports $src_port -m multiport --dports $dest_port -m mark --mark 0/$MMX_MASK -m comment --comment "$1" -j $policy &> /dev/null
				;;
				*)
				[ "$enable" == "1" ] && $IPT4 -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m mark --mark 0/$MMX_MASK -m comment --comment "$1" -j $policy &> /dev/null
				;;
			esac
			...
			case $proto in
				tcp|udp)
				[ "$enable" == "1" ] && $IPT6 -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m multiport --sports $src_port -m multiport --dports $dest_port -m mark --mark 0/$MMX_MASK -m comment --comment "$1" -j $policy &> /dev/null
				;;
				*)
				[ "$enable" == "1" ] && $IPT6 -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m mark --mark 0/$MMX_MASK -m comment --comment "$1" -j $policy &> /dev/null
				;;
			esac
		fi
	fi
