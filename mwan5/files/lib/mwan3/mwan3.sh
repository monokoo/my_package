	...
	if [ -n "$use_policy" ]; then
		...
		if [ "$family" == "any" ]; then
			...
			for IPT in "$IPT4" "$IPT6"; do
				case $proto in
					tcp|udp)
					[ "$enable" == "1" ] && $IPT -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m multiport --sports $src_port -m multiport --dports $dest_port -m mark --mark 0/0xff00 -m comment --comment "$1" -j $policy &> /dev/null
					;;
					*)
					[ "$enable" == "1" ] && $IPT -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m mark --mark 0/0xff00 -m comment --comment "$1" -j $policy &> /dev/null
					;;
				esac
			done
			...
			case $proto in
				tcp|udp)
				[ "$enable" == "1" ] && $IPT4 -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m multiport --sports $src_port -m multiport --dports $dest_port -m mark --mark 0/0xff00 -m comment --comment "$1" -j $policy &> /dev/null
				;;
				*)
				[ "$enable" == "1" ] && $IPT4 -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m mark --mark 0/0xff00 -m comment --comment "$1" -j $policy &> /dev/null
				;;
			esac
			...
			case $proto in
				tcp|udp)
				[ "$enable" == "1" ] && $IPT6 -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m multiport --sports $src_port -m multiport --dports $dest_port -m mark --mark 0/0xff00 -m comment --comment "$1" -j $policy &> /dev/null
				;;
				*)
				[ "$enable" == "1" ] && $IPT6 -A mwan3_rules -p $proto -s $src_ip -d $dest_ip $ipset -m mark --mark 0/0xff00 -m comment --comment "$1" -j $policy &> /dev/null
				;;
			esac
		fi
	fi
