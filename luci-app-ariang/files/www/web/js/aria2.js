  ...
  return {
    ...
    change_pos: function(gid, pos, how) {
      ARIA2.request("changePosition", [gid, pos, how],
        function(result) {
          ...
          main_alert("alert-info", "移动中", 1000);
          ARIA2.refresh();
        }
      );
    },
    ...
    remove: function(gids) {
      ...
      ARIA2.batch_request("remove", gids,
        function(result) {
          ...
          if (error.length == 0) {
            main_alert("alert-info", "删除中", 1000);
            ARIA2.refresh();
          } else {
            main_alert("alert-error", error.join("<br />"), 3000);
          }
    ...
    change_options: function(gid, options) {
      ARIA2.request("changeOption", [gid, options],
        function(result) {
          ...
          main_alert("alert-info", "设置更新", 1000);
        }
      );
    },
    ...
    pause_all: function() {
      ARIA2.request("pauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "暂停所有任务. 请等待例如种子连接到tracker.", 2000);
        }
      );
    },
    ...
    unpause_all: function() {
      ARIA2.request("unpauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "取消所有暂停任务.", 2000);
        }
      );
    },
    ...
    purge_download_result: function() {
      ARIA2.request("purgeDownloadResult", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "删除所有完成的/错误的/清除的下载任务.", 2000);
        }
      );
    },
      ...
      ARIA2.request("changeGlobalOption", [options],
        function(result) {
          if (!result.result)
            ...
            main_alert("alert-success", "保存中", 2000);
        }
      );
    },
    ...
    global_stat: function() {
      ARIA2.request("getGlobalStat", [],
        function(result) {
          ...
          var title = "下载"+YAAW.tpl.view.format_size_0()(result.downloadSpeed);
          if (result.uploadSpeed > 0)
            title += " 上传"+YAAW.tpl.view.format_size_0()(result.uploadSpeed);
          title += " - Aria2 Web 控制台";
    ...
    get_version: function() {
      ARIA2.request("getVersion", [],
        function(result) {
          ...
          $("#global-version").text("Aria2 "+result.result.version+" 运行中" || " aria2连接中...");
        }
      );
    },
    ...
    auto_refresh: function(interval) {
      ...
      interval_id = window.setInterval(function() {
        ...
        if (select_lock) {
          if (need_refresh) {
            main_alert("", "任务列表上一次已经更新. 点击 '刷新' 更新任务列表.");
          }
        } else {
          if (need_refresh) {
            ARIA2.refresh();
